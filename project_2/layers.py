...
class Layer:
    ...
    def get_kaiming_gain(self):
        ...
        '''
        if self.activation == 'relu':
            return tf.cast(tf.math.sqrt(2.0), tf.float32)
        elif self.activation == 'tanh':
            return 5.0 / 3.0
        else:
            return 1.0
...
class Dense(Layer):
    ...
    def init_params(self, input_shape):
        ...
        if self.wt_init == 'he':
            stddev = self.get_kaiming_gain() * tf.math.sqrt(2.0 / tf.cast(input_feats, tf.float32))
        else:
    ...
    def compute_group_norm(self, net_in, eps=0.001):
        ...
        '''
        B = tf.shape(net_in)[0]
        H = net_in.shape[-1]  # static, known at trace time

        num_groups = self.num_groups if self.num_groups is not None else max(8, H // 8)
...
class Flatten(Layer):
    ...
    def compute_net_input(self, x):
        ...
        self.output_shape = [x.shape[0], int(tf.reduce_prod(x.shape[1:]))]
